...
public class categoryDAO  extends DBConn{
	...
	public ArrayList<productVO> categoryList(String type){
		...
		try {
			String sql = "select pid , pname,pinfo,pkind, to_char(price, '9,999,999'), color, simg1,simg2, to_char(pdate,'yy/mm/dd') pdate , pinfo, pkind from (select * from product order by pdate desc) where pkind =?";
	...
	public ArrayList<productVO> priceList(String type){
		ArrayList<productVO> list = new ArrayList<>();
		try {
			String sql = "select pid , pname,pinfo,pkind, to_char(price, '9,999,999'), color, simg1,simg2, to_char(pdate,'yy/mm/dd') pdate , pinfo, pkind from (select * from product order by price desc) where pkind =?";
			getPreparedStatement(sql);
			pstmt.setString(1, type);
			rs = pstmt.executeQuery();
			while(rs.next()) {
